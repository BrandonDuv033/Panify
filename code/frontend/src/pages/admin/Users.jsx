...
export default function Users() {
  ...
  const handleGuardarEdicion = async (datosActualizados) => {
    ...
    if (usuario) {
      ...
      const usuarioCreado = await crearUsuario(datosActualizados);
      setUsuarios((usuariosActuales) => [...usuariosActuales, usuarioCreado]);
    }

    modalEditar.cerrar();

    Swal.fire(
      usuario ? "Usuario actualizado" : "Usuario agregado",
      "Los cambios se guardaron correctamente.",
      "success",
    );
  };

  const handleAgregar = () => modalEditar.abrir();
        ...
        <section className="panel tabla p-4 bg-white rounded shadow-sm">
          <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            ...
            <button className="btn btn-primary" onClick={handleAgregar}>
              <i className="fa-solid fa-plus me-1"></i> Agregar Usuario
            </button>
          </div>
