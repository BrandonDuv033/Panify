...
let productosPanaderia = [
  { id: "P-01", nombre: "Pan Blandito x10", precio: 5000, stock: 20, img: "https://placehold.co/150?text=Pan+Blandito" },
  { id: "P-02", nombre: "Pan Rollito de Queso", precio: 2500, stock: 15, img: "https://placehold.co/150?text=Pan+Queso" },
  { id: "P-03", nombre: "Croissant de Chocolate", precio: 3800, stock: 12, img: "https://placehold.co/150?text=Croissant" },
  { id: "P-04", nombre: "Dona con Chispas", precio: 3000, stock: 18, img: "https://placehold.co/150?text=Dona" }
];
...
function meterAlCarrito(id) {
  ...
  if (existente) {
    ...
    miCarrito.push({ id: prod.id, nombre: prod.nombre, precio: prod.precio, cantidad: 1 });
  }
...
function despacharAlLocalStorage() {
  ...
  const stringProductos = miCarrito.map(c => `${c.cantidad}x ${c.nombre}`).join(", ");
  const granTotal = miCarrito.reduce((acc, el) => acc + (el.precio * el.cantidad), 0);
  ...
  alert("¡Orden enviada con éxito! Ya la puedes revisar en tu panel de administración.");
}